...
export async function createMongoClient(config) {
    ...
    const uri = (_a = config === null || config === void 0 ? void 0 : config.uri) !== null && _a !== void 0 ? _a : process.env.MONGODB_URL;
    if (!uri) {
        throw new ConnectionError('MongoDB URI is required (MONGODB_URL environment variable or config.mongo.uri)');
    }
    try {
        ...
        connectionPromise = null;
        throw new ConnectionError('Failed to connect to MongoDB', error instanceof Error ? error : undefined);
    }
}
/**
